...
export default function Tarefas({ tarefas, poloLabels, corPolo, filtros, setFiltros, pagina, setPagina, onAbrirBitrix, advogados }) {
  const patch = (p) => { setFiltros({ ...filtros, ...p }); setPagina(1); };
  ...
  const filtradas = tarefas.filter((t) => {
    ...
    if (filtros.buscaTexto.trim()) {
      const alvo = [t.clienteNome || '', t.equipeCobrancaColaboradorNome || '', String(t.digitoCpfCliente ?? ''), t.equipeCobrancaAdvogado || ''].join(' ').toLowerCase();
  ...
  return (
    <div style={s('max-width:1360px;animation:fadeSlideIn 0.4s ease both;margin:0 auto;')}>
      <div style={s('margin-bottom:20px;')}>
        ...
        <div style={s('font-size:13px;color:rgba(236,230,216,0.5);margin-top:4px;')}>Lista de tarefas por cliente, roteada automaticamente pelo dígito final do CPF/CNPJ.</div>
      </div>
      ...
      <div style={s('background:#111111;border:1px solid rgba(199,199,199,0.16);border-radius:12px;padding:18px;margin-bottom:18px;')}>
        <div style={s('display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:16px;')}>
          ...
          <div>
            ...
            <select value={filtros.digitoCpf} onChange={(e) => patch({ digitoCpf: e.target.value })} style={s(FIELD)}>
              ...
              {Array.from({ length: 10 }, (_, d) => <option key={d} value={String(d)}>CPF Final {d}</option>)}
            </select>
          </div>
        </div>
        <div style={s('margin-bottom:14px;')}>
          <label style={s(LABEL)}>Tarefa (individual por nome, CPF ou data)</label>
          <input type="text" placeholder="Digite para buscar por nome da tarefa, CPF, ID ou data..." value={filtros.buscaTexto} onChange={(e) => patch({ buscaTexto: e.target.value })} style={s(FIELD)} />
        </div>
        <div style={s('display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;')}>
          <label style={s('display:flex;align-items:center;gap:8px;font-size:12.5px;color:rgba(236,230,216,0.75);cursor:pointer;')}>
            <input type="checkbox" checked={filtros.apenasConcluidas} onChange={() => patch({ apenasConcluidas: !filtros.apenasConcluidas })} style={s('width:16px;height:16px;accent-color:#846419;')} />
            Filtrar apenas concluídas
          </label>
          <button className="btn-ghost" onClick={() => { setFiltros({ advogado: 'todos', escalao48h: 'todos', digitoCpf: 'todos', buscaTexto: '', apenasConcluidas: false }); setPagina(1); }} style={s('display:flex;align-items:center;gap:8px;background:transparent;color:rgba(236,230,216,0.65);border:1px solid rgba(199,199,199,0.3);border-radius:8px;padding:9px 16px;font-size:12.5px;font-weight:600;cursor:pointer;font-family:inherit;')}>
            <IconFilter />
            Limpar filtros
          </button>
        </div>
      ...
      <div style={s('font-size:11.5px;color:rgba(236,230,216,0.4);margin-bottom:10px;')}>Mostrando {filtradas.length} de {tarefas.length} tarefas</div>
      ...
      <div style={s('background:#111111;border:1px solid rgba(199,199,199,0.16);border-radius:12px;overflow:hidden;')}>
        <table style={s('width:100%;border-collapse:collapse;font-size:13px;')}>
          <thead>
            <tr style={s('background:rgba(255,255,255,0.03);')}>
              {['Cliente', 'Polo', 'Colaborador', 'Advogado', 'Critério', 'Status', 'Prazo'].map((h) => <th key={h} style={s(TH)}>{h}</th>)}
              ...
              return (
                <tr key={t.id} className="task-row" onClick={() => onAbrirBitrix(t)} style={s('border-bottom:1px solid rgba(199,199,199,0.08);cursor:pointer;')}>
                  ...
                  <td style={s('padding:10px 16px;text-align:center;')}>
                    <button className="icon-btn" title="Abrir no Bitrix24" onClick={(e) => { e.stopPropagation(); onAbrirBitrix(t); }} style={s('background:transparent;border:1px solid rgba(199,199,199,0.25);color:rgba(236,230,216,0.6);border-radius:6px;padding:6px;cursor:pointer;display:inline-flex;')}>
                      <IconExternal />
                    </button>
                  </td>
                </tr>
        ...
        <div style={s('display:flex;justify-content:space-between;align-items:center;padding:14px 16px;border-top:1px solid rgba(199,199,199,0.12);')}>
          <span style={s('font-size:11.5px;color:rgba(236,230,216,0.45);')}>Página {atual} de {totalPaginas}</span>
